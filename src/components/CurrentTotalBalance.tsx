...
/**
 * @component
 * A React component that displays the current total balance with options to switch between balance types
 * and change the target currency.
 *
 * @returns {JSX.Element} A JSX element containing the total balance, balance type buttons, and a currency selector.
 *
 * @example
 * ```tsx
 *     <CurrentTotalBalance
 ...
 * ```
 */
export default function CurrentTotalBalance({
  setCurrency,
  currentCurrency,
  ...
  return (
    <div className="flex flex-col gap-5 min-w-fit min-h-fit grow widget-border">
      <div className="flex flex-row gap-2 w-fit">
        <button
          className={`py-1 px-2 border border-gray-300 rounded-xl text-xs ${
            balanceType == "AVAILABLE" ? "bg-gray-100" : "hover:shadow-sm"
          } `}
          value="AVAILABLE"
          onClick={onTypeClick}
        >
        ...
        <button
          className={`py-1 px-2 border border-gray-300 rounded-xl text-xs ${
            balanceType == "BOOKED" ? "bg-gray-100" : "hover:shadow-sm"
          } `}
          value="BOOKED"
          onClick={onTypeClick}
        >
      ...
      <div className="flex flex-col gap-2 w-fit">
        ...
        <div className="flex items-center gap-1.5">
          <p className="text-sm font-semibold text-gray-500">Current cash in</p>
