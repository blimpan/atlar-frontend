...
/**
 * @component
 * A React component that displays a line chart representing total balances over time,
 * with data fetching from session storage or API.
 *
 * @returns {JSX.Element} A div containing a LineChart component with dynamically updated labels and values.
 *
 * @example
 * ```tsx
 * <TotalBalanceGraph currentCurrency="USD" />
 * ```
 */
export default function TotalBalanceGraph({
  currentCurrency,
}: Readonly<ComponentProps>) {
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [totalBalancesByDate, setTotalBalancesByDate] = useState<
    Record<string, number>
  >({});
  const [localTargetCurrency, setLocalTargetCurrency] =
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (storageBalances) {
          // We already have total balances by date

          localTotalBalancesByDate = (await JSON.parse(
            storageBalances
          ...
          for (const account of allAccounts) {
            const balancesByDateForAccount = await getBalancesByDate(
              account.id
            );

            for (const date in balancesByDateForAccount) {
              localTotalBalancesByDate[date] =
                (localTotalBalancesByDate[date] || 0) +
                balancesByDateForAccount[date];
            }
          }
          sessionStorage.setItem(
            "totalBalancesByDate",
            JSON.stringify(localTotalBalancesByDate)
          );
        }
