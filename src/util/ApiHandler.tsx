...
/**
 * Fetches all available account balances of specified type.
 *
 ...
 */
export async function getCurrentBalanceForAll(
  balanceType: string,
  limit: number = 20
) {
 ...
 * @throws An error if the API request fails.
 */
export async function getSpecificCurrentBalance(
  accountId: string,
  balanceType: string
) {
...
/**
 * Fetches a list of transactions for a specific account with the given reconciliation status.
 *
 * @param accountId - The unique identifier of the account for which transactions are to be retrieved.
 * @param reconciliationStatus - The reconciliation status filter for the transactions (e.g., "reconciled", "unreconciled").
 * @param limit - The maximum number of transactions to retrieve. Defaults to 25 if not specified.
